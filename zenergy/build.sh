...
say ""
say "${MAGENTA}${BOLD}╔═════════════════════════════════════╗${NC}"
say "${MAGENTA}${BOLD}║   ◈  zenergy kmod build            ║${NC}"
say "${MAGENTA}${BOLD}╚═════════════════════════════════════╝${NC}"
say ""
...
info "Installing akmod-zenergy..."
dnf install -y --setopt=install_weak_deps=False akmod-zenergy
ok "akmod-zenergy installed"

# ── Build kmod ────────────────────────────────────────────────
info "Building zenergy kmod for ${KERNEL_VERSION}..."
akmods --force --kernels "${KERNEL_VERSION}" --kmod zenergy
ok "zenergy kmod built"
...
[[ ${#RPMS[@]} -gt 0 ]] || \
    (cat /var/cache/akmods/zenergy/*.failed.log 2>/dev/null; \
     fail "No zenergy kmod RPM found in /var/cache/akmods/zenergy/")
...
info "Downloading zenergy companion packages..."
...
say ""
say "${MAGENTA}${BOLD}╔═════════════════════════════════════════════╗${NC}"
say "${MAGENTA}${BOLD}║   ◆  zenergy build complete                ║${NC}"
say "${MAGENTA}${BOLD}╚═════════════════════════════════════════════╝${NC}"
say ""